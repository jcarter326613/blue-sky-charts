...
export class TileCache {
    ...
    public getPathForTile(subMapDescription: SubMapDescription, targetZoom: number, x: number, y: number): string {
        ...
        if ( !existsSync(pngFilePath) ) {
            ...
            execSync(`python3 ../geotiff-map-exploder/setup_map.py -use-defaults ${subMapDescription.name} ${subMapDescription.version} sectional`)
        }
        console.log(`Exploding map ${subMapDescription.name}`)
        execSync(`python3 ../geotiff-map-exploder/explode_maps.py ${subMapDescription.name} ${subMapDescription.version} relative mosaic ${targetZoom}`)
